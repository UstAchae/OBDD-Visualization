...
object TempParser {
  ...
  private final class Parser(raw: String) {
    ...
    def parseXor(): Either[ParseError, Expr] =
      leftChain(parseOr, parseXorOp, Xor.apply)

    def parseOr(): Either[ParseError, Expr] =
      leftChain(parseAnd, parseOrOp, (a, b) => if (lastOrWasNor) Nor(a, b) else Or(a, b))

    def parseAnd(): Either[ParseError, Expr] =
      leftChain(parseNot, parseAndOp, (a, b) => if (lastAndWasNand) Nand(a, b) else And(a, b))
    private var lastAndWasNand: Boolean = false
    private var lastOrWasNor: Boolean = false
    ...
    private def parseOrOp(): Option[() => Unit] = {
      skipWs()
      if (peek("||")) Some(() => { i += 2; lastOrWasNor = false })
      else if (peek("|")) Some(() => { i += 1; lastOrWasNor = false })
      else if (peek("∨")) Some(() => { i += 1; lastOrWasNor = false })
      else if (peek("↓")) Some(() => { i += 1; lastOrWasNor = true })
      else if (peek("⊽")) Some(() => { i += 1; lastOrWasNor = true })
      else None
    }
    ...
    private def parseAndOp(): Option[() => Unit] = {
      skipWs()
      if (peek("&&")) Some(() => { i += 2; lastAndWasNand = false })
      else if (peek("&")) Some(() => { i += 1; lastAndWasNand = false })
      else if (peek("∧")) Some(() => { i += 1; lastAndWasNand = false })
      else if (peek("↑")) Some(() => { i += 1; lastAndWasNand = true })
      else if (peek("⊼")) Some(() => { i += 1; lastAndWasNand = true })
      else None
    }
    ...
    private def peekConstTrue(): Boolean = {
      skipWs()
      peekIgnoreCase("true") || peek("⊤")
    }

    private def peekConstFalse(): Boolean = {
      skipWs()
      peekIgnoreCase("false") || peek("⊥")
    }
